...
import { ExchangedPaginated, StudentProps } from '../../shared/interfaces/students'
...
const Remarcacoes:React.FC = () =>{
  ...
  const {data:studentExchangeNoPaginated} = useQuery<StudentProps[]>(['exchangesNoPaginated'], async () =>{
  ...
  return(
    <Container>
      ...
      <DefaultModal
        ...
        setIsOpen={() =>{setHasOpen({...hasOpen,hasOpen:true})}}
        content={
          <div className='h-96 flex flex-col items-center'>
            <div className=' flex justify-between w-full'>
              <h1 className='text-center mb-5'>
                Atualizar dia da reposição
              </h1>
              <span onClick={() =>{setHasOpen({hasOpen:false})}}><AiOutlineClose cursor={'pointer'}/></span>
            </div>

            <CreateAula>
              <Form ref={formRef} className='flex items-center justify-center flex-col h-72'  onSubmit={handleSubmit}>
                <Input type={'date'} name='date' />
                <button type='submit'  className=" mt-5 btn text-white border-none   bg-secundary hover:bg-secundaryOpacity w-full">Atualizar Reposição</button>
      ...
      <Pagination changePage={changePage} current_page={studentExchange?.meta.current_page || 0} first_page={studentExchange?.meta.first_page || 0} last_page={studentExchange?.meta.last_page || 0} />
