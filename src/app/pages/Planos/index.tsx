...
const Planos: React.FC = () => {
  ...
  const { data: plans, isLoading } = useQuery<PlanProps[]>(
    ['plans'],
    async () => {
  ...
  const { isFetching: isFetchignPlan } = useQuery<PlanProps>(
    ['plan', id],
    async () => {
      if (id) {
    ...
    { onSuccess: (data: PlanProps) => id && setPlan(data) }
  );
  ...
  const handleSubmit = async (data: PlanData) => {
    try {
      ...
      const schema = Yup.object().shape({
        ...
        amount_installments: Yup.string().required(
          'Quantidade de meses obrigatorio'
        ),
        ...
        content={
          <div className=" flex flex-col items-center">
            <div className=" flex justify-between w-full">
              <h1 className="text-center font-bold mb-5 w-full">
                {!id ? 'Cadastrar plano' : 'Atualizar plano'}
              </h1>
              <span
                onClick={() => {
                  setOpenModal(false);
              ...
              >
                {isFetchignPlan ? (
                  ...
                  <div>
                    ...
                    <div className="alertMessage ">
                      <label htmlFor="" className="text-xs">
                        Quantidade de meses referentes ao plano
                      </label>
                      <Input
      ...
      <main>
        <Filters className="">
          ...
          <button
            ...
            className="btn text-white border-none  bg-secundary hover:bg-secundaryOpacity"
          >
            CADASTRAR PLANO
          </button>
        </Filters>
